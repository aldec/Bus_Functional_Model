...
//
//  
//	Model:		CY7C2663KV18 8M X 18 QDR(tm)-II+ Burst-of-4 SRAM
// 	Date:		 August 2015
//
//	Revision:	2.0
//
...
//
//	Revision History
//	Revision History 1.0 : New model created
//		  			1.1 : Corrected JTAG ID
//                  1.2: Changed Output Timings
...
// Output times
`define tco	#0.3			// K clock rise to output data valid
...
// Output times
`define tco	#0.27			// K clock rise to output data valid
`define tdoh	#0.30			// Min. Data Output Hold BEFORE C/C# clock rise
`define tccqo #0.30
`define tcqoh #0.30
`define tcqd #0.15
...
input 	[20:0] 		A;		// address busses for a 1024K RAM

output			CQ, CQb;	// Echo Clock Out
output [`wordsize:0]	Q;
...
reg [20:0] Read_Address, Read_Address_o, Read_Address_o_o, Read_Address_o_o_o, Read_Address_o_o_o_o, Write_Address, Write_Address_o, Write_Address_o_o, Write_Address_o_o_o;
reg [20:0] Read_Address_o_o_o_o_o; 
reg [20:0] Write_Address_int; 
...
begin
	...
	//		$display(" burst 2 ", $time);
		...
		begin
			Q = `tco mem4[Read_Address_o_o_o_o_o]; 
...
begin
		...
		begin
			Q =  `tco mem1[Read_Address_o_o]; // added to account for extra latency in QDR2+
   //		   $display(" burst 1 ", $time);
		...
		begin
			Q = `tco mem3[Read_Address_o_o_o_o]; // added to account for extra latency in QDR2+
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010010001010011000001101001;	// IDCODE for QDR2+ CY7C2663KV18 device. This may not be changed.
